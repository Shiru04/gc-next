...
export function FloatingCta() {
  ...
  return (
    <div className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] right-4 z-40 flex items-center gap-3 md:hidden">
      ...
      <a href={es ? "/es/programar-servicio/" : "/schedule-service/"} className="rounded-2xl bg-brand-black px-4 py-3 text-sm font-extrabold text-white shadow-xl transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-red focus-visible:ring-offset-2" aria-label={es ? "Solicitar servicio" : "Request service"} {...ctaAttrs({ id: "floating-schedule", location: "floating", type: "booking" })}>{es ? "Solicitar servicio" : "Request Service"}</a>
    </div>
  );
}
