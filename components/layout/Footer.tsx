...
export function Footer() {
  ...
  return <footer className="border-t border-black/10 bg-white"><Container className="py-12"><div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
    <div><div className="text-lg font-extrabold">{BUSINESS.name}</div><div className="mt-3 text-sm text-black/70"><div>{BUSINESS.addressLine1}</div><div>{BUSINESS.cityStateZip}</div><div className="mt-2"><span>{es ? "Oficina: " : "Office: "}</span><a href={`tel:${BUSINESS.officePhoneE164}`} className="font-semibold hover:text-black hover:underline">{BUSINESS.officePhoneDisplay}</a></div><div className="mt-2">{BUSINESS.hoursShort}</div><div className="mt-3 text-black/60">{BUSINESS.trustLine} | {BUSINESS.licenseLabel}</div></div></div>
