...
export function ContactFormSpanish() {
  ...
  if (status === "sent") return <div role="status" className="py-8 text-center"><h2 className="text-2xl font-extrabold text-brand-red">Solicitud recibida</h2><p className="mt-2 text-black/70">Gracias; recibimos su solicitud. Nuestro equipo se comunicará con usted para confirmar el siguiente paso adecuado.</p></div>;
  ...
  return <form method="post" action="/api/contact/" onSubmit={submit} onFocus={() => { if (!started.current) { started.current = true; pushConversionEvent("contact_form_start", { serviceType: form.service, ctaLocation: "contact-form" }); } }} noValidate className="space-y-4">
    ...
    <div><label htmlFor="cf-service" className="font-semibold">Servicio *</label><select id="cf-service" name="service" value={form.service} onChange={(e) => update("service", e.target.value as ServiceType)} className={fieldClass}>{SERVICE_TYPES.map((type) => <option key={type} value={type}>{SERVICE_LABELS.es[type]}</option>)}</select></div>
