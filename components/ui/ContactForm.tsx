...
export function ContactForm() {
  ...
  if (status === "sent") return <div role="status" className="py-8 text-center"><h2 className="text-2xl font-extrabold text-brand-red">Request received</h2><p className="mt-2 text-black/70">Thanks — we received your request. Our team will contact you to confirm the right next step.</p></div>;
  ...
  return <form method="post" action="/api/contact/" onSubmit={submit} onFocus={() => { if (!started.current) { started.current = true; pushConversionEvent("contact_form_start", { serviceType: form.service, ctaLocation: "contact-form" }); } }} noValidate className="space-y-4">
    ...
    <div><label htmlFor="cf-name" className="font-semibold">Name *</label><input id="cf-name" name="name" autoComplete="name" value={form.name} onChange={(e) => update("name", e.target.value)} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "cf-name-error" : undefined} className={fieldClass} />{error("name")}</div>
    <div><label htmlFor="cf-email" className="font-semibold">Email *</label><input id="cf-email" name="email" type="email" inputMode="email" autoComplete="email" value={form.email} onChange={(e) => update("email", e.target.value)} aria-invalid={Boolean(errors.email)} className={fieldClass} />{error("email")}</div>
    ...
    <div><label htmlFor="cf-service" className="font-semibold">Service *</label><select id="cf-service" name="service" value={form.service} onChange={(e) => update("service", e.target.value as ServiceType)} className={fieldClass}>{SERVICE_TYPES.map((type) => <option key={type} value={type}>{SERVICE_LABELS.en[type]}</option>)}</select></div>
    <div><label htmlFor="cf-message" className="font-semibold">How can we help? *</label><textarea id="cf-message" name="message" rows={5} value={form.message} onChange={(e) => update("message", e.target.value)} aria-invalid={Boolean(errors.message)} className={fieldClass} />{error("message")}</div>
