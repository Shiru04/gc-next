...
export function customerFlowHref(service: ServiceType, locale: "en" | "es" = "en", coupon?: string) {
  ...
  if (["ac_repair", "heating_repair", "maintenance", "commercial_repair", "commercial_maintenance"].includes(service)) return `/request-service/?service=${encodeURIComponent(service === "heating_repair" ? "ac_repair" : service)}`;
