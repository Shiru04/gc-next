...
export const PROJECT_TYPES = ["replace_existing_system", "new_system_installation", "not_sure"] as const;
...
export type InstallationInput = {
  ...
  systemType: typeof SYSTEM_TYPES[number]; timeline: typeof TIMELINES[number];
  financingInterest: typeof FINANCING_INTERESTS[number]; comments: string; consent: boolean;
  turnstileToken: string; website: string; attribution: Record<string, unknown>;
};
...
export function validateInstallation(value: unknown): { success: true; data: InstallationInput } | { success: false; errors: InstallationErrors } {
  ...
  const data: InstallationInput = {
    ...
    projectType: oneOf(raw.projectType, PROJECT_TYPES, "not_sure"), systemType: oneOf(raw.systemType, SYSTEM_TYPES, "not_sure"),
    timeline: oneOf(raw.timeline, TIMELINES, "researching"), financingInterest: oneOf(raw.financingInterest, FINANCING_INTERESTS, "maybe"),
    comments: text(raw.comments, 2000), consent: raw.consent === true, turnstileToken: text(raw.turnstileToken, 2048),
    website: text(raw.website, 120), attribution: raw.attribution && typeof raw.attribution === "object" ? raw.attribution as Record<string, unknown> : {},
  };
